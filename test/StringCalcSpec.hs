...
testSuites =
  [ ("simple expressions",         [ ((Map.empty, "1"),       ("1", Map.empty))
                                   , ((Map.empty, ""),        ("Invalid input", Map.empty))
                                   , ((Map.empty, "2l"),      ("Invalid input", Map.empty))
                                   ])
  , ("single binary expressions",  [ ((Map.empty, "1+1"),     ("2", Map.empty))
  ...
  , ("complex binary expressions", [ ((Map.empty, "-1"),      ("-1", Map.empty))
                                   , ((Map.empty, "3+2*2"),   ("7", Map.empty))
                                   , ((Map.empty, "(3+2)*2"), ("10", Map.empty))
                                   , ((Map.empty, " (      3+ 2   ) * 2   "), ("10", Map.empty))
                                   ])
  , ("variables",                  [ ((Map.empty, "let x = 12"),       ("12", fromList [("x", 12)]))
